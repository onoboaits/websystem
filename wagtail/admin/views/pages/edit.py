...
from home.utils import get_req_tenant_id
...
class EditView(TemplateResponseMixin, ContextMixin, HookResponseMixin, View):
    ...
    def publish_action(self):
        ...
        if not ENABLE_WAGTAIL_PUBLISH:
            submit_wagtail_page_revision_to_compliance(self.request.user, revision, self.page)

            messages.success(
                self.request, _(
                    "The page has been submitted for review by compliance"
                ), buttons=[]
            )
    ...
    def submit_action(self):
        # Disable functionality entirely
        return self.redirect_away()
